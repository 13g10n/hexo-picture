...
function buildPublishedAssetPath(assetPath) {
  // eslint-disable-next-line no-undef
  const path = hexo.config.root + Url.resolve('/', assetPath);
  return path.replace(/\/{2,}/g, '/');
}

/**
 * Get configurations for picture tag render
 * @param {Object} post_ Current processed post
 * @param {Array} args Arguments given to tag
 * @returns {Object} Picture tag config
 */
function getTagConfig(post_, args) {
  let config = { post: post_, classes: [], alt: [] };
  args.forEach((arg) => {
    if (!config.asset) {
      const asset = PostAsset.findOne({ post: post_._id, slug: arg });
      if (asset) {
        config.asset = asset;
      } else {
